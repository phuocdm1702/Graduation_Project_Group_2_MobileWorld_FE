                    ...
                    <div v-for="invoice in filteredPendingInvoices" :key="invoice.id"
                      ...
                      <div class="card-body p-2" style="
                        ...
                        <div class="d-flex gap-3 justify-content-between align-items-center mb-2">
                          <span class="fw-bold text-dark">{{ invoice.ma }}</span>
    ...
    <div class="row g-4">
      <div class="col-lg-8">
        ...
        <FilterTableSection title="Giỏ hàng" icon="bi bi-basket">
          <div class="shadow-sm product-card" style="border-bottom-right-radius: 12px; border-bottom-left-radius: 12px">
        ...
        <FilterTableSection title="Thông tin đơn" icon="bi bi-info-circle" class="flex-fill">
          <div class="order-card h-100">
            ...
            <div class="card-body p-3 pt-0 d-flex flex-column">
              ...
              <div v-if="cartItems.length === 0" class="empty-cart-message text-center py-4">
                <div
                  class="empty-icon-container rounded-circle mx-auto mb-3 d-flex align-items-center justify-content-center gradient-custom-teal"
              ...
              <div v-else class="d-flex flex-column flex-grow-1">
                <div v-if="isDelivery" class="mb-5">
                  <h6 class="fw-bold text-dark mb-3" style="font-size: 1.3rem; letter-spacing: 0.5px">
                ...
                <div class="voucher-section mb-5">
                  ...
                  <div class="voucher-container bg-white p-3 rounded-3 shadow-sm animate__animated animate__fadeIn"
                    ...
                    <div class="price-info mt-4 p-3 rounded-3 shadow-sm animate__animated animate__fadeInUp"
                ...
                <div>
                  ...
                  <div class="d-flex flex-wrap gap-2 mb-3">
                    <button class="btn px-4 py-2 select-btn" @click="selectPayment('transfer')" :class="{
                      'gradient-custom-teal text-white': paymentMethod === 'transfer',
                    ...
                    <button class="btn px-4 py-2 select-btn" @click="selectPayment('cash')" :class="{
                      'gradient-custom-teal text-white': paymentMethod === 'cash',
                      'border': paymentMethod !== 'cash'
                    }" style="border: 1px solid rgba(52, 211, 153, 0.2)">
                      Tiền mặt
                    </button>
                    <button class="btn px-4 py-2 select-btn" @click="selectPayment('both')" :class="{
                      'gradient-custom-teal text-white': paymentMethod === 'both',
                ...
                <!-- Pay Button -->
                <button class="btn w-100 py-3 bg-custom-teal-btn text-white mt-auto" @click="ThanhToan"
                ...
                <div class="row align-items-center">
                  <div class="col-md-4 text-center">
                    <img :src="selectedCartItem?.image ||
                      ...
                      " class="phone-img img-fluid rounded" :alt="selectedCartItem?.name"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
          <div class="modal-content shadow-lg p-3 gradient-modal animate__animated animate__zoomIn"
                ...
                <div class="row align-items-center">
                  <div class="col-md-4 text-center">
                    <img :src="selectedProduct?.image ||
                      'https://bachlongstore.vn/vnt_upload/product/04_2024/Untitled_2.png'
                      " class="phone-image img-fluid rounded" :alt="selectedProduct?.tenSanPham"
                      style="max-height: 150px; object-fit: contain; transition: transform 0.3s ease"
                      @mouseover="this.style.transform = 'scale(1.05)'" @mouseout="this.style.transform = 'scale(1)'" />
                  </div>
                  <div class="col-md-8">
            ...
            <div class="modal-footer border-0">
              ...
              <button class="btn btn-light px-4 py-2 gradient-custom-teal text-white" @click="addProductWithIMEIs"
          ...
          <div class="modal-content shadow-lg p-3 gradient-modal animate__animated animate__zoomIn"
            ...
            <div class="modal-body p-4">
              ...
              <div v-if="activeTab === 'private'">
                <div v-if="privateDiscountCodes.length === 0" class="text-center text-muted py-4">
                  <i class="bi bi-info-circle me-2"></i>Không có mã giảm giá cá nhân nào.
                </div>
                <div v-else class="d-flex flex-column gap-3">
                  <div v-for="code in privateDiscountCodes" :key="code.id" class="discount-card p-3 rounded shadow-sm"
                    style="background: #fff; border: 1px solid rgba(52, 211, 153, 0.1)">
                    <div class="d-flex justify-content-between align-items-center">
                      <div>
                        <h6 class="fw-bold text-dark mb-1">{{ code.code }}</h6>
                        <p class="text-muted mb-0">Giảm: {{ formatPrice(code.value) }}</p>
                      </div>
                      <button class="btn btn-sm gradient-custom-teal text-white"
              ...
              <div v-if="activeTab === 'public'">
                <div v-if="publicDiscountCodes.length === 0" class="text-center text-muted py-4">
                  <i class="bi bi-info-circle me-2"></i>Không có mã giảm giá công khai nào.
                </div>
                <div v-else class="d-flex flex-column gap-3">
                  <div v-for="code in publicDiscountCodes" :key="code.id" class="discount-card p-3 rounded shadow-sm"
                    style="background: #fff; border: 1px solid rgba(52, 211, 153, 0.1)">
                    <div class="d-flex justify-content-between align-items-center">
                      <div>
                        <h6 class="fw-bold text-dark mb-1">{{ code.code }}</h6>
                        ...
                        <p class="text-muted mb-0">
                          Hết hạn: {{ new Date(code.expiry).toLocaleDateString('vi-VN') }}
                        </p>
                      </div>
                      <button class="btn btn-sm gradient-custom-teal text-white"
            ...
            <div class="modal-footer border-0">
              <button class="btn btn-light px-4 py-2" @click="showDiscountModal = false">
                Đóng
              </button>
            </div>
          </div>
            ...
            <div class="modal-body p-4">
              <div class="row g-4">
                ...
                <div class="col-md-6">
                  ...
                  <select v-model="newCustomer.city" class="form-select shadow-none" @change="handleProvinceChange">
                    <option value="" disabled>Chọn tỉnh/thành phố</option>
                    <option v-for="province in provinces" :key="province.code" :value="province.name">
                      {{ province.name }}
                    </option>
                ...
                <div class="col-md-6">
                  ...
                  <select v-model="newCustomer.district" class="form-select shadow-none" @change="handleDistrictChange"
                ...
                <div class="col-md-6">
                  ...
                  <select v-model="newCustomer.ward" class="form-select shadow-none" :disabled="!newCustomer.district">
                    <option value="" disabled>Chọn phường/xã</option>
                    <option v-for="ward in wards" :key="ward.code" :value="ward.name">
                      {{ ward.name }}
                    </option>
...
import BanHang from './BanHang.js';
