<template>
  <div class="py-4">
    <!-- Header -->
    <HeaderCard title="Quản Lý Bán Hàng" badgeText="Hệ Thống POS" badgeClass="teal"
        ...
        <FilterTableSection title="Khách Hàng" icon="bi bi-people" class="flex-fill">
          <div class="customer-card bg-white rounded-3 shadow-sm animate__animated animate__fadeIn flex-fill">
            <div class="card-body p-4 d-flex flex-column h-100">
              ...
              <button class="btn teal text-white w-100 mt-3 py-2 fw-medium add-customer-btn"
                @click="openCustomerModal" style="
                  border-radius: 10px;
                  transition: all 0.3s ease;
                  font-size: 1rem;
    ...
    <div class="row">
      <div class="col-lg-8">
        ...
        <FilterTableSection title="Sản Phẩm Trong Giỏ hàng" icon="bi bi-basket">
          <div class="shadow-sm product-card" style="border-bottom-right-radius: 12px; border-bottom-left-radius: 8px;">
            ...
            <div class="card-body p-4">
              ...
              <div v-if="!filteredCartItems || filteredCartItems.length === 0" class="empty-cart-container text-center py-5">
                <div
                ...
                <p class="text-muted mb-0 fw-medium">Giỏ hàng trống hoặc không tìm thấy sản phẩm</p>
              </div>
                      ...
                      <div class="col-md-5">
                        ...
                        <div v-if="selectedDiscount" class="d-flex align-items-center gap-2">
                          ...
                          <span class="badge bg-success text-white px-3 py-1">
                            {{ selectedDiscount.code }} (-{{ formatPrice(selectedDiscount.value) }})
                          </span>
                        </div>
                      </div>
                      <!-- Giá và hành động -->
        ...
        <div v-if="showProductDetails" class="product-details-modal shadow-lg p-3 animate__animated animate__fadeIn"
          ...
          <div class="modal-content bg-white rounded-3 p-4">
            ...
            <div class="row">
              <div class="col-md-6">
                <p><strong>Mã sản phẩm:</strong> {{ selectedCartItem?.maSanPham || 'N/A' }}</p>
              ...
              <div class="col-md-6">
                <p><strong>Giá bán:</strong> {{ formatPrice(selectedCartItem?.currentPrice) }}</p>
                <p><strong>Số lượng:</strong> {{ selectedCartItem?.quantity }}</p>
                <p><strong>IMEI:</strong> {{ selectedCartItem?.imei || 'N/A' }}</p>
                <p><strong>Thương hiệu:</strong> {{ selectedCartItem?.brand || 'N/A' }}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="mt-4">
            ...
            <DataTable title="" :headers="productHeaders" :data="filteredProducts"
              ...
              <template #actions="{ item }">
                <button class="btn btn-sm px-4 py-2 add-to-cart-btn teal text-white"
                    ...
                    <div class="row g-3">
                      ...
                      <div class="col-12">
                        ...
                        <div class="input-group">
                          ...
                          <select v-model="selectedShippingUnit" class="form-select shadow-none border-start-0"
                            @change="updateShippingFee" style="
                              border-radius: 0 8px 8px 0;
                              transition: all 0.3s ease;
                            ">
                            <option value="" disabled>
                            ...
                            <option v-for="unit in shippingUnits" :key="unit.id" :value="unit">
                              {{ unit.name }}
                            </option>
                          </select>
                        </div>
                        <!-- Hiển thị logo đơn vị vận chuyển -->
                        <div v-if="selectedShippingUnit" class="mt-3 d-flex justify-content-center">
                          <div class="p-3 border rounded shadow-sm bg-white" style="
                              max-width: 200px;
                              width: 100%;
                              text-align: center;
                            ">
                            <img :src="selectedShippingUnit.logo" :alt="selectedShippingUnit.name" style="
                                max-height: 90px;
                                object-fit: contain;
                                width: 100%;
                              " />
                            <p class="mt-2 mb-0 fw-semibold text-dark" style="font-size: 0.9rem">
                      ...
                      <div class="col-12">
                        ...
                        <div class="input-group">
                          ...
                          <input v-model.number="shippingFee" type="number"
                            class="form-control shadow-none border-start-0" placeholder="Nhập phí vận chuyển"
                            :disabled="totalPrice >= FREE_SHIP_THRESHOLD" min="0" style="
                              border-radius: 0 8px 8px 0;
                              transition: all 0.3s ease;
                            " />
                        </div>
                        <div v-if="totalPrice >= FREE_SHIP_THRESHOLD" class="text-success mt-2">
      ...
      <div v-if="showIMEIModal" class="modal fade show d-block" tabindex="-1" style="background: rgba(0, 0, 0, 0.5); gap: 0;" >
        <div class="modal-dialog modal-dialog-centered modal-lg">
          <div class="modal-content shadow-lg p-3 gradient-modal animate__animated animate__zoomIn" style="
              background: rgba(255, 255, 255, 0.95);
              backdrop-filter: blur(15px);
...
.delete-invoice-btn:hover, .btn-close-custom:hover {
  background: #ef4444;
  color: white;
  border-color: #ef4444;
}
