  ...
  <div class="container-fluid py-4">
          ...
          <div class="d-flex gap-3 overflow-x-auto pb-2">
            <div v-for="invoice in filteredPendingInvoices" :key="invoice.id" @click="loadPendingInvoice(invoice)"
              class="card shadow-sm bill-card p-2 cursor-pointer min-w-[160px] flex-shrink-0" :style="{
                ...
                border:
                  activeInvoiceId === invoice.id ? '2px solid #0052cc' : 'none',
              }">
    ...
    <div class="row g-4">
      <div class="col-lg-8">
        <FilterTableSection title="Giỏ hàng" icon="bi bi-basket">
          <div class="shadow-sm product-card">
            <div class=" d-flex justify-content-end">
              <button class="btn add-to-cart-btn gradient-custom-blue text-white" @click="scanQR"
                style="width: 120px; margin-top: 1rem; margin-right: 1rem;" :disabled="!activeInvoiceId">
        ...
        <FilterTableSection class="mt-4" title="Danh Sách Sản Phẩm" icon="bi bi-table">
          ...
          <div class="mt-4">
            ...
            <DataTable title="" :headers="productHeaders" :data="filteredProducts" :pageSizeOptions="[5, 10]"
              ...
              <template #name="{ item }">
                <span class="fw-medium" style="color: #002962">{{
                  item.name
                }}</span>
              </template>
              <template #color="{ item }">
        ...
        <div v-if="showCartIMEIModal" class="modal fade show d-block" tabindex="-1"
          ...
          <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content p-2 gradient-modal" style="
              ...
              <div class="modal-header border-0 d-flex justify-content-between align-items-center">
                <h5 class="modal-title fw-bold text-dark">
                  IMEI của {{ selectedCartItem?.name }} ({{
        ...
        <div v-if="showIMEIModal" class="modal fade show d-block" tabindex="-1" style="background: rgba(0, 0, 0, 0.5)">
          <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content shadow-lg p-2 gradient-modal" style="
                background: rgba(255, 255, 255, 0.95);
                backdrop-filter: blur(15px);
                border-radius: 12px;
              ">
              <div class="modal-header border-0 d-flex justify-content-between align-items-center">
                <h5 class="modal-title fw-bold text-dark">
                  Chọn IMEI cho {{ selectedProduct?.name }} ({{
              ...
              <div class="modal-body p-4">
                  ...
                  <div v-for="item in availableIMEIs" :key="item.id" class="imei-card mb-3 p-3 bg-light rounded">
                    <div class="d-flex justify-content-between align-items-center">
                      ...
                      <div>
                        ...
                        <span v-if="selectedIMEIs.includes(item.imei)" class="badge bg-success text-white">Đã
                          chọn</span>
                      </div>
                    </div>
                  </div>
                ...
                <div v-if="selectedIMEIs.length > 0"
                  ...
                  <h6 class="selected-imei-title fw-semibold text-dark mb-3">
                    IMEI đã chọn:
                  </h6>
                  <div class="selected-imei-list d-flex flex-wrap gap-2">
                    <div v-for="imei in selectedIMEIs" :key="imei"
                      ...
                      <span class="imei-text fw-medium text-dark">{{
                        imei
                      }}</span>
        ...
        <div v-if="showDiscountModal" class="modal fade show d-block" tabindex="-1"
          style="background: rgba(0, 0, 0, 0.5)">
          <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content shadow-lg gradient-modal" style="
                background: rgba(255, 255, 255, 0.95);
                backdrop-filter: blur(15px);
                border-radius: 16px;
              ">
              ...
              <div class="modal-body p-4 discount-modal-body">
                ...
                <div v-show="activeTab === 'private'" class="discount-section animate__animated animate__fadeIn">
                  <h6 class="fw-semibold text-dark mb-4 text-center">
                    Mã Giảm Giá Riêng
                  </h6>
                      ...
                      <div class="card shadow-sm discount-card h-100 animate__animated animate__zoomIn">
                        <div class="card-body d-flex flex-column justify-content-between p-3">
                          <div class="d-flex justify-content-between align-items-start">
                            <div>
                              <h6 class="discount-code fw-bold text-primary mb-1">
                                {{ code.code }}
                              </h6>
                              <p class="mb-1">
                                Giảm
                                <span class="highlight-text fw-semibold text-success">{{ formatPrice(code.value)
                                }}</span>
                              </p>
                            </div>
                            <span class="badge bg-primary text-white px-2 py-1">Riêng</span>
                          </div>
                          <button class="btn btn-sm gradient-custom-blue text-white btn-apply w-100 mt-3" @click="
                        ...
                        <div class="card-body d-flex flex-column justify-content-between p-3">
                          <div class="d-flex justify-content-between align-items-start">
                            <div>
                              ...
                              <p class="mb-1">
                                Giảm
                                <span class="highlight-text fw-semibold text-success">{{ code.percent }}%</span>
                              </p>
                              <p class="mb-1">
                                Đơn tối thiểu:
      ...
      <div class="col-lg-4">
        <!-- Customer Info -->
        <FilterTableSection title="Khách Hàng" icon="bi bi-people">
          <div class=" shadow-sm customer-card">
            <div class="card-body p-4">
              <div class="mb-3">
                <div class="input-group">
              ...
              <button class="btn w-100 mb-3 add-customer-btn gradient-custom-blue text-white"
              ...
              <div v-if="selectedCustomer" class="row g-3">
                <div class="col-md-6">
                  ...
                  <input v-model="customer.name" type="text" class="form-control shadow-none"
                    placeholder="Nguyễn Oanh" />
                </div>
                <div class="col-md-6">
                  <label class="form-label fw-medium text-dark">Số điện thoại</label>
                  <input v-model="customer.phone" type="text" class="form-control shadow-none"
        ...
        <!-- Order Info -->
        <FilterTableSection title="Thông tin đơn" icon="bi bi-info-circle">
          <div class="shadow-sm order-card">
            ...
            <div class="card-body p-4">
              ...
              <div class="mb-4">
                ...
                <div class="d-flex flex-wrap gap-2">
                  <button @click="selectPayment('transfer')" class="btn px-4 py-2" :class="{
                    'gradient-custom-blue text-white':
                      paymentMethod === 'transfer',
                    border: paymentMethod !== 'transfer',
                  }" style="border: 1px solid rgba(0, 94, 226, 0.2)">
                    Chuyển khoản
                  </button>
                  <button @click="selectPayment('cash')" class="btn px-4 py-2" :class="{
                    'gradient-custom-blue text-white': paymentMethod === 'cash',
                    border: paymentMethod !== 'cash',
                  }" style="border: 1px solid rgba(0, 94, 226, 0.2)">
                    Tiền mặt
                  </button>
                  <button @click="selectPayment('both')" class="btn px-4 py-2" :class="{
                    'gradient-custom-blue text-white': paymentMethod === 'both',
              ...
              <button class="btn w-100 py-3 pay-btn gradient-custom-green text-white" @click="ThanhToan" :disabled="!activeInvoiceId || cartItems.length === 0 || isCreatingOrder
                ">
